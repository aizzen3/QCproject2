...
for number_of_layers in layer_list:
    ...
    if number_of_layers == 0:
        ...
        # ------------------------------------------------------------
        # Manual fix: layer 0 = target state itself.
        #
        # The library's own number_of_layers=0 circuit is NOT the
        # identity (it already contains a full brick-wall of two-qubit
        # unitaries), so U_psi(0)^dagger |psi> != |psi>. Since we want
        # layer 0 to represent "no disentangling applied yet", we set
        # chi_0 = psi manually instead of trusting the library here,
        # and report zero circuit depth/size for this point.
        # ------------------------------------------------------------
        ...
        depth_this_layer = U_psi.depth()
        size_this_layer = U_psi.size()
    ...
    circuit_depth_results.append(depth_this_layer)
    circuit_size_results.append(size_this_layer)
    ...
    print("Circuit depth:", depth_this_layer)
    print("Circuit size:", size_this_layer)
